metadata = {
    "protocolName": "Protein Quantification and Normalization - Part 2: Normalized Sample Preparation False!",
...
def add_parameters(parameters):
    ...
    parameters.add_int(
        ...
        default=1,
        choices=[{"display_name": "24 Tube Rack with 1.5mL Tubes", "value": 1},
    ...
    parameters.add_float(
        ...
        default=1,
        choices=[{"display_name": "1x", "value": 1}, {"display_name": "0.5x", "value": 0.5},
...
def run(ctx):
    ...
    for i in [std_1, std_2, std_3, std_4, std_5, std_6, dilution_1,dilution_2]:
        ctx.comment(str(i))
    ...
    for l in range(6):
        ...
        if l > 0:
            if od_std[l] > od_std[l - 1]:
                raise Exception("Invalid Standards", str())
    ...
    if sample_labware == 1:
        if num_sample > 24:
            def_1 = ctx.define_liquid(name="Samples", description="Samples, per tube or well (Slot C1)",
                                      display_color="#FF0000")  ## Red
            def_2 = ctx.define_liquid(name="Samples", description="Samples, per tube or well (Slot B1)",
                                      display_color="#FF0000")  ## Red
            for p in range(24):
            ...
            def_1 = ctx.define_liquid(name="Samples", description="Samples, per tube (Slot C1)",
                                      display_color="#FF0000")  ## Red
            for p in range(num_sample):
                sample_rack_1.wells()[p].load_liquid(liquid=def_1, volume=vol_unkwn / num_sample)
    else:
        def_1 = ctx.define_liquid(name="Samples", description="Samples, per well (Slot C1)",
                                  display_color="#FF0000")  ## Red
        for p in range(num_sample):
            sample_rack_1.wells()[p].load_liquid(liquid=def_1, volume=vol_unkwn / num_sample)

    vol_dilu = 5000
    def_dilu = ctx.define_liquid(name="Diluent", description="Buffer for dilution, per tube",
                                 display_color="#8B8000")  ## Yellow
