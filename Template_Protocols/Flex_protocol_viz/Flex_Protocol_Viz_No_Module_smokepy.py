...
def run(protocol: protocol_api.ProtocolContext):
    ...
    pipette.detect_meniscus(plate["A1"])
        # 1. Detect the meniscus in the source well
    # This returns a Meniscus object which stores the height data
    source_meniscus = pipette.detect_meniscus(plate["A1"])

    # 2. Aspirate relative to the detected height
    # Use the 'source_meniscus' object directly
    pipette.aspirate(100, source_meniscus.bottom(z=-4)) 
    # put liquid back in the source well
    pipette.dispense(50, source_meniscus.top(z=-1))
    pipette.dispense(50, source_meniscus.top(z=-2))
    pipette.return_tip()
