...
metadata = {
    'protocolName': 'Customizable Serial Dilution customized for LC',
    'author': 'QA - Alex',
...
def run(protocol_context):
    ...
    pipette.pick_up_tip()
    dilutent_LC = protocol_context.define_liquid_class('water')
    dilutent_LC_config = dilutent_LC.get_for(pipette, tipracks[0]) 
    dilutent_LC_config.aspirate.retract.air_gap_by_volume.set_for_volume = [diluent_volume, air_gap_volume]
    for dest in all_diluent_destinations:
        ...
        pipette.transfer(
                ...
                new_tip='never')
        ''' 
        protocol_context.comment(f'''
                volume = {str(diluent_volume)}
                source = {str(diluent)}
                dest = {str(dest)}
                air_gap = {str(air_gap_volume)}
                new_tip = 'never'
                ''' ) 
    ...
    sample_liquid_LC = protocol_context.define_liquid_class('ethanol_80')
    sample_liquid_LC_config = sample_liquid_LC.get_for(pipette,tipracks[1])
    sample_liquid_LC_config.dispense.mix.enabled = True 
    sample_liquid_LC_config.dispense.mix.repetitions = 5
    sample_liquid_LC_config.dispense.mix.volume = total_mixing_volume/2
    ...
    for source_set, dest_set in zip(dilution_source_sets,
        ...
        for s, d in zip(source_set, dest_set):
            protocol_context.comment(f'''
                volume = {str(transfer_volume)}
                source = {str(s)}
                dest = {str(d)}
                new_tip = 'never'
                mix_after=(5, {str(total_mixing_volume)}/2)
                ''' ) 
            pipette.transfer_liquid(liquid_class = sample_liquid_LC, volume = transfer_volume, source = s, dest =d, new_tip =tip_use_strategy  )
    ...
    if blank_on:
        ...
        for blank_well in blank_set:
            protocol_context.comment(f'''
                ...
                new_tip = 'never'
                ''' ) 
            pipette.transfer_liquid(liquid_class = dilutent_LC, volume = diluent_volume, source = diluent,dest = blank_well, new_tip = 'never' )
            ''' 
            pipette.transfer(
                    diluent_volume,
                    diluent,
